...
namespace System.IO
{
    ...
    internal static class Win32Marshal
    {
        ...
        internal static Exception GetExceptionForWin32Error(int errorCode, string path)
        {
            switch (errorCode)
            {
                case Interop.Kernel32.Errors.ERROR_FILE_NOT_FOUND:
                    if (path.Length == 0)
                ...
                case Interop.Kernel32.Errors.ERROR_PATH_NOT_FOUND:
                    if (path.Length == 0)
                ...
                case Interop.Kernel32.Errors.ERROR_ACCESS_DENIED:
                    if (path.Length == 0)
                ...
                case Interop.Kernel32.Errors.ERROR_FILE_EXISTS:
                    if (path.Length == 0)
                        goto default;

                    return new IOException(SR.Format(SR.IO_FileExists_Name, path), MakeHRFromErrorCode(errorCode));

                case Interop.Kernel32.Errors.ERROR_OPERATION_ABORTED:
