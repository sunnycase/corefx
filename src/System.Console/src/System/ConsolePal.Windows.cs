...
namespace System
{
    ...
    internal static class ConsolePal
    {
            ...
            get
            {
                ...
                while (true)
                {
                    ...
                    if (!r)
                    {
                        ...
                        if (errorCode == Interop.Kernel32.Errors.ERROR_INVALID_HANDLE)
        ...
        private static Interop.Kernel32.CONSOLE_SCREEN_BUFFER_INFO GetBufferInfo(bool throwOnNoConsole, out bool succeeded)
        {
            ...
            {
                int errorCode = Marshal.GetLastWin32Error();
                if (errorCode == Interop.Kernel32.Errors.ERROR_INVALID_HANDLE && !throwOnNoConsole)
                    return new Interop.Kernel32.CONSOLE_SCREEN_BUFFER_INFO();
                throw Win32Marshal.GetExceptionForWin32Error(errorCode);
            }
        ...
        private sealed class WindowsConsoleStream : ConsoleStream
        {
            ...
            public override int Read([In, Out] byte[] buffer, int offset, int count)
            {
                ...
                if (Interop.Kernel32.Errors.ERROR_SUCCESS != errCode)
            ...
            private unsafe static int ReadFileNative(IntPtr hFile, byte[] bytes, int offset, int count, bool isPipe, out int bytesRead, bool useFileAPIs)
            {
                ...
                {
                    bytesRead = 0;
                    return Interop.Kernel32.Errors.ERROR_SUCCESS;
                }
                ...
                int errorCode = Marshal.GetLastWin32Error();
                if (errorCode == Interop.Kernel32.Errors.ERROR_NO_DATA || errorCode == Interop.Kernel32.Errors.ERROR_BROKEN_PIPE)
                    return Interop.Kernel32.Errors.ERROR_SUCCESS;
                return errorCode;
            }
